...
def preprocess_lstm_df(df):
    """
    Preprocess air pollution dataset for LSTM training/prediction.
    
    Steps:
    - Strip column names
    - Drop/clean NaNs in features
    - Convert numeric columns to float
    - Derive timestamp-based features: hour, day, month, weekday
    - Return cleaned dataframe
    """
    df = df.copy()
    
    # Strip column names and string columns
    df.columns = [c.strip() for c in df.columns]
    for c in df.select_dtypes(include='object').columns:
        df[c] = df[c].astype(str).str.strip()
    
    # Convert numeric columns to float (replace commas if any)
    numeric_cols = [
    ...
    # Build timestamp & derive time features if 'year','month','day','hour' exist
